...
function readMounts() {
  // /proc/mounts est résolu dans le namespace de montage DU LECTEUR : monter
  // /proc de l'hôte ne suffit donc pas. En revanche /proc/1/mounts décrit le
  // namespace du PID 1 — c'est-à-dire celui de l'hôte quand on lit son procfs.
  const sources = [path.join(PROC, '1/mounts'), path.join(PROC, 'mounts')];
...
function readDisks() {
  ...
  for (const m of mounts) {
    ...
    // Rien à afficher pour les montages internes à Docker ni pour les greffons.
    if (/^\/(var\/lib\/docker|run\/docker|snap)\//.test(m.mount)) continue;
...
function readNetDev() {
  // /proc/net/dev est résolu dans le namespace réseau du lecteur : on lit donc
  // celui du PID 1 (l'hôte) pour obtenir le trafic réel de la machine.
  const raw = readFile(path.join(PROC, '1/net/dev')) || readFile(path.join(PROC, 'net/dev'));
  if (!raw) return [];
  const out = [];
  for (const line of raw.split('\n').slice(2)) {
...
/**
 * Relit la table de montage DU CONTENEUR pour vérifier ce qui a réellement été
 * monté en /host/* et si c'est bien en lecture seule. Sert de preuve à l'écran.
 */
function readHostMountInfos() {
  ...
  for (const line of raw.split('\n')) {
    ...
    // On ne décrit que les trois montages demandés, pas leurs sous-montages.
    if (!mountPoint || !/^\/host\/[^/]+$/.test(mountPoint)) continue;
    const options = (parts[5] || '').split(',');
    const sep = parts.indexOf('-');
    out.push({
      path: mountPoint,
      readOnly: options.includes('ro'),
      fstype: sep > 0 ? parts[sep + 1] : null,
    });
  }
  return out;
}
...
function collect() {
  ...
  if (!usingHostMounts) {
    const missing = [];
    if (PROC !== HOST_PROC) missing.push('/host/proc');
    if (SYS !== HOST_SYS) missing.push('/host/sys');
    if (ROOT !== HOST_ROOT) missing.push('/host/root');
    warnings.push(
      `Montage(s) hôte absent(s) : ${missing.join(', ')}. CPU et RAM restent ceux de la machine ` +
      '(compteurs /proc non namespacés) ; la liste des disques et des processus se limite à ce que voit le conteneur.'
    );
  } else if (!readHostMountInfos().some((m) => m.readOnly)) {
    warnings.push('Montages hôte présents mais en lecture-écriture : préférez :ro.');
  }
  ...
  const net = readNetDev().filter((n) => !/^veth/.test(n.iface)).map((n) => {
    ...
    // Cartes physiques d'abord, puis tunnels, puis ponts Docker ; à rang égal,
    // l'interface la plus active en tête.
    const rank = (i) => (/^(eth|en|wl|bond|em)/.test(i) ? 0 : /^(tun|tap|wg|tailscale)/.test(i) ? 1 : 2);
    return rank(a.iface) - rank(b.iface)
